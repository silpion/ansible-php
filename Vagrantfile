...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  if ENV['ANSIBLE_LIB_VAGRANT_BOXNAME']
    config.vm.box = ENV['ANSIBLE_LIB_VAGRANT_BOXNAME']
  else
  ...
  config.vm.define :ansiblelibtest do |d|
    ...
    d.vm.provision :ansible do |ansible|
      ...
      ansible.verbose = ENV['ANSIBLE_LIB_VAGRANT_ANSIBLE_VERBOSE']
    ...
    d.vm.provider :virtualbox do |v|
      ...
      v.customize [ 'modifyvm', :id, '--name', 'ansiblelibtest', '--memory', '512', '--cpus', '1' ]
    end
